...
部署脚本会自动配置防火墙，但如果你使用云服务商（如阿里云、腾讯云、AWS），需要在**安全组**中手动开放以下端口：

| 端口 | 协议 | 用途 | 是否必需 |
|------|------|------|---------|
| **80** | TCP | HTTP（自动重定向到 HTTPS） | ✅ 必需 |
| **443** | TCP | HTTPS（Web 访问） | ✅ 必需 |

**重要提示**：
- 如果使用域名，必须开放 80 和 443 端口
- 如果只用 IP 访问，只需开放 80 端口
- 部署脚本会自动配置服务器防火墙（UFW/firewalld）
- 云服务商的安全组需要**手动配置**

#### 如何访问

**使用域名访问（推荐）**：
1. 在 DNS 中将域名 A 记录指向你的服务器 IP
2. 部署时选择"配置域名"
3. 访问：`https://your-domain.com`
4. 首次访问会看到 Authelia 登录页面
5. 输入部署时生成的用户名和密码
6. 登录成功后自动跳转到 OpenClaw 控制面板

**使用 IP 访问**：
1. 部署时选择"跳过域名配置"
2. 访问：`http://your-server-ip`（注意是 HTTP，不是 HTTPS）
3. 同样会看到登录页面
4. 输入用户名密码登录
...
**登录凭据位置**：
- 部署完成后会显示在终端
- 保存在 `/root/openclaw-access-info.txt`
- 或运行 `./manage.sh` 选择"查看访问信息"
